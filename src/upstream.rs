...
impl Upstream {
    ...
    pub fn health_url(&self) -> Result<Url, url::ParseError> {
        self.base_url.join(&self.health_path)
    }
}
...
impl UpstreamPool {
    pub fn new(configs: &[UpstreamConfig]) -> Self {
        Self {
            upstreams: configs
                .iter()
                .map(|config| Arc::new(Upstream::new(config)))
                .collect(),
    ...
    pub fn any_ready(&self) -> bool {
        ...
        self.upstreams
            .iter()
            .any(|upstream| upstream.is_healthy() && upstream.breaker.is_available(now))
    }
    ...
    pub fn select(&self) -> Option<Arc<Upstream>> {
        ...
        for offset in 0..candidates.len() {
            ...
            if upstream.breaker.begin_request(now) {
                return Some(upstream);
            }
            if offset + 1 >= candidates.len() {
                break;
            }
        }
        None
...
mod tests {
    ...
    fn excludes_unhealthy_upstreams() {
        let pool = UpstreamPool::new(&[
            upstream("a", "http://127.0.0.1:1", 1),
            upstream("b", "http://127.0.0.1:2", 1),
        ]);
